...
yday_months_x <- scale_x_discrete(breaks = cumsum(c(1, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)), labels = c(month.name, "January"))
yday_months_y <- scale_y_discrete(breaks = cumsum(c(1, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)), labels = c(month.name, "January"))
```
...
# later analysis.
usable_sales <- sales %>% filter(amt_is_uncertain == FALSE & has_double_price == FALSE & transaction_type %in% c("sold", "bought in", "sold or bought in"))
```
...
```{r per_year}
per_year <- ggplot(usable_sales, aes(x = year)) +
  geom_histogram(binwidth = 1) +
  theme_bw() +
...
```{r per_month}
per_month <- ggplot(usable_sales %>% mutate(month = factor(month, labels = month.name)), aes(x = month)) +
  theme_bw() +
  geom_histogram() +
  theme(legend.position = "bottom") +
...
usable_sales %>% 
  mutate(
    ...
    is_christies = auction_house == "Christie's") %>% 
  filter(auction_house %in% c("Christie's", "Foster (Edward)")) %>% 
  ggplot(aes(x = wday)) +
  theme_bw() +
  facet_wrap(~auction_house) +
  geom_histogram()
```
...
cv_results <- bind_rows(
  ...
  group_by(set) %>% 
  do(cv_trends(., n_days = n_days)) %>% 
  filter(type == "count_cv") %>% 
  rename(price_bin = price_factor) %>% 
  ggplot(aes(x = year, y = cv, color = estimate)) +
  geom_point(size = 3, alpha = 0.5) +
  geom_smooth(method = "lm", se = TRUE) +
  facet_grid(set ~ price_bin) +
...
dualplot(cv_results, "cv_results", h = 7, w = 9)
```
...
ra_overlap <- bind_rows(
  ...
  mutate(
    after_start = yday >= exhib_start_day,
    during_exhib = yday >= exhib_start_day & yday <= exhib_end_day) %>% 
  filter(price_factor == 5)

ggplot(ra_overlap, aes(x = year, weight = transaction_amt, fill = during_exhib)) +
  geom_density(position = "fill", alpha = 0.6) +
  scale_fill_brewer(type = "qual", palette = 6) +
  facet_wrap(~set) +
  theme_bw()

ra_ribbon <- ra_overlap %>% 
  group_by(set, year) %>% 
  mutate()
...
ra_plot <- ggplot(ra_overlap, aes(x = year, y = yday)) +
  ...
  geom_ribbon(aes(ymin = exhib_start_day, ymax = exhib_end_day, na.rm = TRUE), alpha = 0.0) +
...
Did Christie's navigate with houses like Fosters and Phillips to find slots on the calendar year?

```{r auction_house_shuffle}
houses <- usable_sales %>%
  filter(
    auction_house %in% c("Christie's", "Foster (Edward)", "Phillips (Harry)") &
      price_factor == 5) %>% 
...
geom_jitter (aes(y = yday, color = auction_house), size = 4, alpha = 0.5) +
  geom_line(data = ra_dates, aes(x = exhib_year, y = exhib_start_day), size = 1, color = "black") +
  geom_line(data = ra_dates, aes(x = exhib_year, y = exhib_end_day), size = 1, color = "black") +
  yday_months_y +
  theme_bw() +
  scale_color_brewer(type = "qual", palette = 6) +
  xlim(1780, 1835) +
  theme(legend.position = "bottom") +
  labs(x = NULL, y = paste("date of top", n_days, "days"), color = "auction house")
