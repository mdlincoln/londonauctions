...
year. To visualize what this spread might look like, take the following violin
plot, where the density of sales over the course of 6 different years are
plotted along the y-axis, moving from `yday` 1 (January 1st) to `yday` 365
(December 31st), and the width of each shape marks the relative density of sales
at that point in the year. Note that for several such as 1780-1800, the sales
are relatively spread out up to around day 200 of the year, while for 1810-1830,
the years "bulge" with a concentration of sales. The relative spread of sales
for each year has also been marked by color.

```{r density, fig.cap = "How concentrated in the year are each of these sales?"}
sales %>% 
  filter(price_factor == 5 & year %in% seq(1780, 1830, 10)) %>% 
  group_by(year) %>% 
  mutate(spread = sd(yday)/mean(yday)) %>% 
  ggplot(aes(x = factor(year), y = yday, fill = spread)) +
  geom_violin(alpha = 0.7) +
  ylim(1, 365) +
  theme_bw() +
  theme(legend.position = "bottom") +
  labs(y = "Day of the year", x = NULL)
```
...
by the mean of all days) confirms this visual impression. As the top sales
concentrate closer and closer together, the CV for each year decreases
accordingly.
