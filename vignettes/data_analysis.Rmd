...
write_csv(cv_combined, path = "../bas_export/cv_combined.csv")
...
houses <- usable_sales %>% 
  ...
  filter(price_factor %in% c(1:5), auction_house != "Other") %>% 
  group_by(price_factor, auction_house) %>% 
  do(peak_n(df = ., n_days = 7)) %>% 
  ungroup()

jh <- houses %>% inner_join(usable_sales, by = c("auction_house", "price_factor", "year", "yday")) %>% 
  select(auction_house, year, price_factor, yday, metric, objnum) %>% 
  nest(objnum) %>% 
  jsonlite::toJSON()

format_jh <- jh %>% 
  as.character() %>%
  jqr::jq(".[] | {auction_house: .auction_house, year: .year, yday: .yday, price_factor: .price_factor, metric: .metric, objects: [.data[].objnum]}")
write(format_jh, file = "bas_export/nested_houses.json")

write_csv(sample_n(usable_sales, 100), path = "bas_export/sample_sales.csv")
write_csv(houses, path = "bas_export/houses.csv")
write_csv(ra_dates, path = "bas_export/ra_dates.csv")
