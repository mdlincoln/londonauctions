...
per_month <- ggplot(usable_sales %>% mutate(month = factor(month, labels = month_names)), aes(x = month)) +
  theme_bw() +
  geom_histogram() +
  theme(legend.position = "bottom") +
...
house_dist <- ggplot(houses, aes(x = year)) +
  geom_jitter(aes(y = yday, color = auction_house), size = 3, alpha = 0.7) +
...
dualplot(house_dist, "house_dist", h = 7)
```