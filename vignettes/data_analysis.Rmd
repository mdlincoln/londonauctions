...
weekday_dist <- usable_sales %>% 
  ...
  mutate(
    ...
    auction_house = factor(auction_house, levels = c("Christie's", "Phillips (Harry)", "Foster (Edward)"))) %>% 
  ggplot(aes(x = wday, fill = auction_house)) +
  geom_histogram(alpha = 0.9) +
  theme_bw() +
  scale_fill_brewer(type = "qual", palette = 6, guide = "none") +
  facet_wrap(~ auction_house, nrow = 1) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "day of the week", y = "total sales, 1780-1835")
