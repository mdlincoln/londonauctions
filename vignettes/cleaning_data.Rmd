...
base_sales <- one_col %>%
  ...
  select(cat_no = `BBF Number`, lot_no = `Lot Number`, auction_house = `Auct.House Auth.`, is_edited = `Edit Status`, sale_date = `Lot Sale Date`, sale_begin = `Sale Begin Date`, sale_end = `Sale End Date`, transaction_type = Transaction, transaction = Price, artists = `Verbatim Artist`, title = `Title`, sellers = `Verbatim Seller`, buyers = `Verbatim Buyer`) %>% 
...
sales <- sales %>% 
  select(cat_no, lot_no, auction_house, date, transaction_type, type_is_uncertain, amt_is_uncertain, has_double_price, transaction_amt, year, month, week, yday, mday, wday, transaction, artists, title, sellers, buyers)
```
