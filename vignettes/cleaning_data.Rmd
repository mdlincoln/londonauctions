...
base_sales <- read_csv(system.file("extdata", "all_england.csv", package = "londonauctions"), skip = 1, col_names = correct_cols, col_types = "cccccccccccccccccccccccccc")
...
sales <- sales %>% 
  group_by(year) %>% 
  mutate(pice_factor = ntile(transaction_amt, n = 5)) %>% 
  ungroup()
```
