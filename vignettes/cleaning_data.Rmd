...
base_sales <- one_col %>%
  ...
  select(objnum, sale_no = `BBF Number`, lot_no = `Lot Number`, auction_house = `Auct.House Auth.`, is_edited = `Edit Status`, sale_date = `Lot Sale Date`, sale_begin = `Sale Begin Date`, sale_end = `Sale End Date`, transaction_type = Transaction, transaction = Price)
...
Amounts are generally listed in pounds sterling. There are very few sales with uncertain prices. We will exclude these. There are
also comparatively few lots with more than one price attached. We will also
exclude these.

```{r transaction_amounts}
# Create a separate column denoting uncertainty
...
sales <- sales %>% filter(amt_is_uncertain == FALSE & has_double_price == FALSE)
```
...
sales <- sales %>% 
  select(objnum, sale_no, lot_no, auction_house, date, transaction_type, type_is_uncertain, transaction_amt, year, month, week, yday, mday, wday, price_factor)
```
...
They Getty has not yet input all of its catalogues.

```{r getty_input}
getty_catalogs <- read_csv(system.file("extdata", "catalog_completion.csv", package = "londonauctions")) %>% 
  mutate(is_input = ifelse(is_input == "yes", TRUE, FALSE))
```
